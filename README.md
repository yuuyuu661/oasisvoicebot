...
1. 本人の同意範囲（利用目的、公開範囲、保存期間、削除方法）を記録します。
2. 静かな部屋で、BGM・反響・他人の声がない音声を録音します。
3. 通常の話し方で複数の文章を読み、合計1～5分程度を用意します。
4. 無音、咳、言い直し、クリッピングした部分を除去します。
5. モノラルWAVへ変換します。元データは非公開ストレージへ保存します。

```bash
ffmpeg -i recording.m4a -ac 1 -ar 22050 -c:a pcm_s16le reference.wav
```

Discordの通話録音は、参加者全員の同意と各地域の法令を確認してから使用してください。

### 推奨する生成構成

日本語に対応し、短い参照音声から声色を抽出できるOpenVoice V2を候補にします。
OpenVoiceサービスは次の処理を行います。

1. `reference.wav`から話者埋め込みを抽出します。
2. 埋め込みを`voice-model-id`へ関連付けて、非公開領域へ保存します。
3. Botから受け取った日本語テキストをベース音声へ変換します。
4. 話者埋め込みを適用し、WAVデータを返します。

OpenVoice V2の公式手順ではLinux、Python 3.9、MeloTTS、UniDic、
`checkpoints_v2`を使用します。最初は公式の`demo_part3.ipynb`で
`reference.wav`の品質を確認してから、HTTP API化してください。

RailwayはGPU処理向けのプラットフォームではないため、OpenVoiceサービスは
GPU対応ホストで運用するか、低速でもよければCPUで検証します。
VOICEVOX Engineとは別サービスにし、APIキーとPrivate NetworkまたはHTTPSで保護します。

### Botへ登録

例えば、抽出した話者IDを`yuuyuu-v1`とした場合は以下のように設定します。

```dotenv
CLONE_TTS_URL=https://非公開の音声生成API
CLONE_TTS_API_KEY=十分に長いランダム値
CLONE_VOICES_JSON={"yuuyuu":"yuuyuu-v1"}
```

設定後にBotを再デプロイすると、`/voice`のProviderに
「録音サンプル音声」が追加され、音声欄に`yuuyuu`が表示されます。

音声モデルを削除するときは、参照音声、話者埋め込み、生成キャッシュ、
バックアップを同じ削除手順の対象にしてください。
